...
plot_follower_distributions = False
...
for d, name in zip([df_rel, df_unr, df], ["rel", "unr", "all"]):
    ...
    for usr in sorted(num_cited, key=lambda u: num_cited[u], reverse=True)[:k]:
        ...
        #       end="\\\\\n",
        #       sep=" & ")
        print(usr, num_cited[usr], num_followers[usr]["followers"],
              end=" \\\\\n",
              sep=" & ")
    print("---------")
    ...
    # k = 12
    # for usr in sorted([u for u in num_cited if -1 < num_followers[u]["followers"] < 10000],
